...
Truly flat land is extremely rare over distances of more than a few meters. The cutoff slope value for *effective* flatness is not always provided in soil description standards (e.g. @soilsurveystaffFieldBookDescribing2024) and is not consistent when it is, e.g.
...
Estimate and record the generalised surface cover components as a percentage composition of the site (@sec-pr-site), using the codes in @tbl-pr-surfcov. Consider the surface cover from the surface to a height of 1.35 m, for consistency with @hurst2022. Note the lithology of surface rock fragments or outcrop according to @sec-pm-lith. Optionally, describe surface rock cover in detail according to @sec-rockfrag.
...
dat_pr_surfcov <-
  tribble(~Code, ~Name, ~Description,
          ...
          'BG', 'Small Fragments', ' Rock fragments, shells, or bones, 2–200 mm',
          'BB', 'Large fragments', 'Large rock fragments >200 mm',
          'BR', 'Rock', 'Bare rock outcrop',
...
[GRAPHIC: Turn the above into a grid with labels underneath conceptual diagrams]{.diags}
...
Note that infilled cracks should be described as per @sec-pr-inf.
...
#| tbl-cap: "Drivers of microrelief (adapted from @ncst2024 and @iussworkinggroupwrb2022)"
...
dat_pr_micreltype <-
  tribble(~Code, ~Name, ~Description,
          ...
          'A', 'Anthropogenic', 'Induced by deliberate human activity')
...
Small-scale human alterations to the soil surface are common in agricultural and urban areas (both current and former). Microrelief can be both removed and created by these activities. 

If the alteration is relatively recent, a surface disturbance code (e.g. [MC]{.ceg}) from @tbl-ss-eventp can be used. Otherwise, record the dominant anthropogenic microrelief type using the codes in @tbl-pr-amicreltype. The driver (@tbl-pr-micreltype) in all cases will be [A]{.ceg}. For detailed descriptions of anthropogenic microrelief, record the average height and spacing of repeating elements in centimetres.
