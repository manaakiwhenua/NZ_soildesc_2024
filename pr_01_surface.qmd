...
Precision needs will vary with slope and survey purpose; whole number degrees are fine on any slope over about 5 degrees, but finer gradations can be useful for flatter locations. Slope classes may be applied to the recorded data; some options are presented in @sec-slopeclass.
...
::: {#nte-flatwhat .callout-note collapse="true" title="Defining 'flat' in soil survey"}
...
The slope measurement method in use can restrict the recordable limit of flat land. For instance, a hand-held clinometer may be difficult to use consistently and accurately at slopes of less than 3°, while survey-grade equipment (e.g. a tripod-mounted laser level) will have a higher accuracy. Slope calculated from a DEM also has inherent limitations. For example, a DEM with a cell size of 20 m and a vertical resolution of 0.5 m cannot discriminate slope increments of less than \~1.43° ($tan^{-1}(0.5/20)$).
:::

::: {#nte-slpdem .callout-note collapse="true" title="Extracting slope from DEMs"}
...
Estimate and record the following generalised surface cover components as a percentage composition of the area surrounding a point observation, or the area within a sampling plot. Consider the land cover from the surface to a height of 1.35m, for consistency with @hurst2022.
...
These parameters should be recorded only to the nearest 5%. It will be unusual to encounter more than three components at any one site.
...
```{r}
#| label: tbl-pr-swater
#| tbl-cap: "Surface water character (adapted from WRB 2022)"
...
#| tbl-cap: "Surface water persistence (adapted from WRB 2022)"
...
::: callout-note
Recent weather data is useful for interpreting surface crack information. See @sec-weath-rrain.
:::
...
dat_pr_micreltype <-
  tribble(~Code, ~Name, ~Description,
          ...
          'S', 'Smoothed',    'Landform surface has been smoothed, rock-picked, infilled or levelled, beyond normal tillage effects',
          'R', 'Raised Beds', 'Elevated areas have been created and maintained for specific crops, e.g. raised rows or puke for kūmara',
...
For example, [FL M FR]{.ceg} for a recent flood that has dropped sediment on a low terrace and partly smothered grass, or [NO - -]{.ceg} for an unaffected site. Optionally, the dominant size class of the material can be reported using the codes in [XLINK]{.todo}
...
dat_pr_deposition <-
  tribble(~Code, ~Name, ~Description,
          # water
          'FL', 'Flood', 'Deposition driven by indunation events along streams',
