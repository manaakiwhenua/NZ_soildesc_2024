...
::: {#nte-consnzsc .callout-note collapse=true title='Consistence and the NZSC'}
...
flowchart TD
  q1["Can a sample cube be formed\n from remoulded material?"] -->|yes| q2[Is the material plastic?]
  q2 ---->|yes| a1([Cohesive])
  q1 -->|no| q3[Is the material plastic?]
  q3 -->|yes| q4
  q2 -->|no| q4["Does the material have a\n fast positive reaction to\n the dilatency test?"]
  q4 -->|no| q5["Does the material have a\n distinctly negative reaction to\n the dilatency test?"]
  q4 --->|yes| a2
...
1.  Perform the test on an minimally disturbed 30 mm cube.
...
-   Test at observed field moisture condition. [check]{.todo}
...
Soil strength at the plastic limit has engineering implications. [expand pls]{.todo}
...
[Plasticity is the capacity of soil material to change shape continuously when stress is
applied and to retain that shape after the removal of stress.]{.milne}
...
dat_cn_stik <-
  tribble(~Code, ~Name, ~Description,
          1, 'Non-sticky', 'After release of pressure, practically no soil material
adheres to the thumb or forefinger',
          2, 'Slightly sticky', 'After release of pressure, soil material adheres perceptibly
to both thumb and forefinger but, as the digits are separated, it tends to come off one or the other cleanly. The material is not stretched appreciably when the thumb and finger are separated',
...
Thixotropic materials liquefy when shaken, and solidify when compressed.
...
1. Adjust the soil to a wet state.
2. Roll into a ball ~30 mm diameter.
3. Hold sample in upturned palm and rapidly shake the hand back and forth.

Thixotropic materials will collapse from a ball into a pancake shape when vibrated, but w 
