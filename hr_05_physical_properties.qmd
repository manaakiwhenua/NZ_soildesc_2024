...
The distinction between cohesive and non-cohesive is recognised by performing moulding, plasticity, and dilatency tests in the following sequence (@fig-cn-cohes). Where doubt still remains after these tests have been completed, the dry coherence test (@sec-cn-dryco) may be necessary.
...
Dilatency occurs due to volume change during shearing and is used as a criterion for the distinguishing non-cohesive from cohesive soil materials.
...
A dilatent reaction is shown by a shiny surface caused by water extrusion, and by a dull surface when the palm of the hand is folded to partly remould the sample, causing water to be drawn into the sample. Record dilatent behaviour as a binary, [true/false]{.ceg}.
