...
Codes for primary divisions (previously known as master horizons) can be used by themselves for rapid assessment, or where criteria for more detailed nomenclature is not met. Always capitalised, they may be divided in to three organic horizons (F, H, O) and five mineral horizons (A, E, B, C, R) as per @tbl-hr-horprime.

::: {#nte-lhzn .callout-note appearance="minimal" collapse="true" title="L Horizons"}
Note that while the 'L' horizon was previously available to describe fresh leaf litter, such materials are now not considered part of the profile description and so this designation has been removed. Litter presence is described as part of surface cover assessment (@sec-pr-surfcov).
:::
