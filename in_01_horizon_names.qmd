...
Silt coats, or 'skeletans' may be present and the horizon normally overlies a Bt horizon containing significantly more clay. Where the lower boundary is a lithological discontinuity and there is no clear evidence of an illuvial relationship with the underlying 2B horizon, the notation EBw is used (see Transitional Horizons)")
...
dat_hr_hortwor <-
  tribble(~Code, ~Name, ~segs, ~cols, ~usewith,
          '*no code*', 'Unaffected' , '<2%', 'None', 'Any',
          '(f)', 'Slightly reduced', '≥2%', 'None', 'B, C',
          '(g)', 'Reduced', '≥2%', '<50%', 'E (alone), B, C',
          'g' , 'Very reduced', '≥2%', 'Matrix 50–85%, and/or ped faces >50%', 'A, E (alone), B, C',
          'r', 'Completely reduced', 'not diagnostic', 'Matrix >85%', 'E (alone), B, C')
...
tbl_hr_hortwor <- gt(dat_hr_hortwor) |>
  ...
  cols_label(usewith = 'Used with', segs = 'Redox segregations', cols = 'Low chroma colours') |>
...
Buried horizons may be denoted by the prefix 'b' (e.g., [bAh]{.ceg}). The notation for a buried [Ah]{.ceg} horizon in material similar to the overlying deposit is [bAh]{.ceg}, and that for a buried [Ah]{.ceg} horizon in dissimilar material is [2bAh]{.ceg}. The horizons of a sequence of buried soils are given consecutive numbers for each soil following the prefix [b]{.ceg}, independently of the numbers for lithological discontinuities that are placed before the [b]{.ceg}. @tbl-bureg shows an example of nomenclature for a buried profile.
