...
Estimating particle size in the field involves compensating for the influences described above and arriving at estimates of sand and clay content (silt can be determined by difference). Record clay and sand to the nearest whole percent. Optionally, add a separate error estimate (+/- x%) as an expression of confidence.

## Coarse fraction

The mineral fraction \> 2 mm is still part of the soil macrofabric, but for practical reasons is measured separately. Coarse mineral fractions, or rock fragments, can be classified on a size scale with ranges of $2*10^n - 6*10^n$, as follows @tbl-hr-rockfrsz.
...
-   For detailed assessments, append further information on any or all of dominant fragment lithology using the codes in @tbl-ss-pm and shape using the codes in @tbl-hr-rockfrshp below.
