...
A great deal of soil physical and chemical behaviour is dependant on the particle size distribution within the solid fraction - the relative dominance of particular sizes, as well as the total size range present. This includes both mineral and organic matter. In the field, particle size distribution is estimated on a per-horizon basis using a combination of visual examination and hand manipulation under moist conditions. Field estimates of particle size distribution, particularly of the \< 2 mm fraction, are commonly referred to as 'soil texture'.

While the true particle size distribution is continuous, for ease of communication it is divided up into set ranges, the broadest of which are detailed in @tbl-hr-simpletex. Organic matter is treated slightly differently, in that it can't be separated easily from the soil mass in the field, and its particle size is less important than other characteristics (see @sec-hr-ortxt).
...
dat_hr_simpletex <-
  tribble(~Code, ~Name, ~Description,
         ...
         'O', 'Organic',  'Comprised largely of decomposing organic matter')

tbl_hr_simpletex <- gt(dat_hr_simpletex) |>
  tab_options(
...
The following method is recommended for use in the field. The method can also be used on fresh soil from representative bagged samples if field time is limited. Oven-dried samples are not reliable estimators of texture, as some soil minerals clump together when heated and cannot be redispersed by hand.

1.  Collect a small handful of soil material (\~30 mm block) from the target soil horizon, avoiding macrofeatures and boundary transition zones as much as possible.
2.  Pick or sieve to remove the coarse fraction.
3.  If the sample is not already moist, either add small amounts of water while working the sample to bring its moisture content up, or work it in the hand until it dries. Additional soil might be required if too much water is added at once. Once the correct moisture state is reached, the sample should feel moist to the touch but not release much, if any, water when squeezed.
...
7.  Check the look and feel of the ball. Is it gritty, floury, or buttery? Does it behave like plasticine? Can it be polished to a shiny surface?
...
dat_hr_texchar <-
  tribble(~Group, ~Code, ~Name, ~Features,
          ...
          'Loamy', 'CL', 'Clay Loam', 'Some visible grains but mostly finer material. Feels slightly gritty to the touch. Can form a coherent ball and a ribbon of ~25–50 mm length.',
          ...
          'Silty', 'ZZ', 'Silt', 'Fine material without visible grains. Feels smooth or floury to the touch. May form a weak ball but usually smears when attempting to form a ribbon.',
          'Silty', 'ZL', 'Silty loam', 'Fine material without visible grains. Feels smooth or floury to the touch. Can form a coherent ball and a ribbon of ~25–50 mm length.',
          'Clayey', 'LC', 'Loamy clay', 'Fine material with some appreciable grit. Forms a coherent, smooth ball that may be difficult to compress. Can form a ribbon of > ~50 mm length.',
          'Clayey', 'ZC', 'Silty clay', 'Fine material without visible grains. Forms a coherent, very smooth ball that may be difficult to compress. Can form a ribbon of > ~50 mm length.',
          'Clayey', 'CC', 'Clay', 'Fine material without visible grains. Forms a coherent, smooth ball that may be difficult to compress, but can usually compress without cracking. Can form a ribbon of > ~50 mm length.')
...
Soil texture has four major classes, defined in @tbl-hr-earthgrps. These in turn contain several subdivisions each, for a total of eleven texture classes. The classes are most easily visualised on a ternary diagram, as seen in @fig-texture-tri. The range limits for each class are given in @tbl-texture-tri.
...
dat_hr_earthgrps <-
  tribble(~Code, ~Name, ~Description,
         ...
         'Z', 'Silty',  'Comprised largely of mineral particles 0.002–0.06 mm',
...
dat_hr_textri <- nztt_labels |>
  ...
  dplyr::mutate(txgroup = case_when(cat_texture_code %in% c('S', 'LS') ~ 'Sandy',
                                  cat_texture_code %in% c('Z', 'LZ', 'ZL') ~ 'Silty',
                                  cat_texture_code %in% c('LC', 'ZC', 'C') ~ 'Clayey',
                                  TRUE ~ 'Loamy'),
  ...
  # replace CZS codes (prevent duplication vs group codes)
  mutate(cat_texture_code = ifelse(cat_texture_code == 'S', 'SS', cat_texture_code)) |>
  mutate(cat_texture_code = ifelse(cat_texture_code == 'Z', 'ZZ', cat_texture_code)) |>
  mutate(cat_texture_code = ifelse(cat_texture_code == 'C', 'CC', cat_texture_code)) |>
...
These divisions reflect significant changes in measurable properties like hydraulic conductivity, but it is important to note that there are no sudden and obvious changes in hand-feel at each boundary.

#### Sand fraction modifiers {#sec-hr-sandmods}

The loamy and sandy texture classes in @tbl-texture-tri can optionally be accompanied by additional information about the sand fraction. These codes, listed in @tbl-hr-sandquals, can be appended to a texture class as a prefix, e.g. [KLS]{.ceg} for coarse loamy sand.
...
dat_hr_omtex <-
  tribble(~Percent, ~Convention, ~egcode, ~egtext,
         '< 17%', 'Do not append', 'CL', 'for a clay loam with little organic material',
         '17 - 30%', 'Append as suffix', 'CL(P)', 'for a clay loam with ~20% peaty organic material',
         '30 - 50%', 'Append as prefix', '(P)CL', 'for a clay loam with ~40% peaty organic material',
         '> 50%', 'Use as primary code', 'P', 'for a peat with little to no mineral material')
...
Where total coarse mineral materials occupy more than 5% of the horizon volume, their presence should be signified in the texture code. Use the [G]{.ceg} gravel, [C]{.ceg} cobble, or [B]{.ceg} boulder codes from @tbl-hr-simpletex, choosing the code that describes the majority of the coarse material, and apply it using the conventions in @tbl-hr-cftex.

```{r}
#| label: tbl-hr-cftex
#| tbl-cap: "Coarse mineral material modifier rules"

dat_hr_cftex <-
  tribble(~Percent, ~Convention, ~egcode, ~egtext,
         '< 5%', 'Do not append', 'CL', 'for a clay loam with ~2% gravel',
         '5 - 35%', 'Append as suffix', 'CL(G)', 'for a clay loam with ~10% gravel',
         '35 - 70 ',  'Append as prefix', '(G)CL', 'for a clay loam with ~50% gravel',
         '> 70%', 'Use as primary code', 'G', 'for a clean gravel')
...
tbl_hr_cftex <- gt(dat_hr_cftex) |>
  ...
  tab_style(style = list(cell_text(color  = 'red',
            ...
            locations = cells_body(columns = 3)) |>
  cols_label('egcode' = 'Example', egtext = '') |>
  cols_width(Percent ~pct(10), Convention ~pct(25), egcode ~pct(10))
...
For poorly sorted horizons where a single coarse fraction does not dominate the particle size distribution, choose the largest coarse code for recording and determine coarse material abundance from the total percentage of coarse materials. For example, in a silt loam horizon with 15% gravel, 15% cobble and 10% boulder, use [(C)ZL]{.ceg} (total coarse fraction 40%, largest co-dominant fraction cobble).
...
#### Gravel fraction modifiers

The gravel fraction can optionally be subdivided into fine, medium and coarse gravel, much like the sand fraction (see @sec-hr-sandmods). These codes, listed in @tbl-hr-gvquals, can be appended to the [G]{.ceg} code as a prefix e.g. [KG]{.ceg} for coarse gravel.
...
For rapid assessment of texture, record one of the codes in @tbl-hr-simpletex per horizon. For routine assessment, use the codes in  @tbl-hr-texchar. Where further detail is required, apply the modifiers as described in @sec-hr-texture.
...
As discussed in @wrn-tex, fine-earth texture estimated by hand in the field can include information about other soil characteristics. Estimating particle size from hand texturing requires practice, developing an ability to identify and account for these sources of interference.

Laboratory particle size measurement also has some limitations. Available methods can involve measurements of particle shape and mineralogy, but these methods are labour-intensive and generally confined to a research context. High-throughput analyses are far more common and usually involve removal of organic materials and soluble minerals, as well as more aggressive aggregate dispersion using shaking and chemical solutions. Below approximately 0.04 mm, the method of measurement switches from sieving to less direct observations (commonly related to settling time in a liquid column). The calculations used to translate these observations into particle size estimates make some assumptions about shape and mineralogy that may not hold true for the sample under examination (see @loveland2001, for a more detailed discussion).

For these reasons, field-estimated and laboratory-determined particle size should never be stored in the same fields in a database, nor should one measurement override another. Laboratory particle size analysis should not be used to change field texture codes; rather, a 'laboratory texture' may be generated from the data. It is important to clearly document the origin of particle size estimates .
:::

::: {#nte-psb .callout-note appearance="minimal" collapse="true" title="Particle size fraction boundaries"}
The boundaries of the fine-earth size fractions vary by jurisdiction. The silt-clay boundary is usually consistent within soil science, and aligns with some major changes in particle properties; chief among them the emergence of electrostatic behaviour in the clay fraction. The sand/silt boundary can range from 0.02–0.063 mm. See @tbl-hr-texcompar for the fractions used in New Zealand and elsewhere. Other jurisdictions may also have different approaches to subdividing the major fractions further [@ncst2024, p. 162].
...
Size fraction boundaries also vary by discipline; sedimentologists set the limit of 'clay' at 0.004 mm while colloid chemists use 0.001 mm [@guggenheim1995]. Clay mineralogists use an alternate definition of clay that does not rely on particle size at all, instead highlighting physical behaviour. Alternate soil texture triangles compatible with this concept have been proposed, but cannot be determined in the field [@moreno-maroto2022].
:::

### Recording whole-soil particle size estimates

Percentage estimates of solid components require some adjustment when reporting on a whole-soil basis. Clay, silt and sand percentage estimates are initially reported as a fraction of the mineral fine earth only, and organic matter as a fraction of the total fine earth. Coarse materials are reported as a percentage of the whole soil mass.
...
-   This leaves $85\% - 12.75\% = 72.25\%$ as the fine fraction adjustment:
...
For example, a horizon with a sandy loam fine earth texture of 60% sand, 25% silt, and 15% clay along with 10% cobble would be described at its very simplest as a "loam with cobble", or in more detail as a "sandy loam with cobble". If the cobble occupied 50% of the total solid volume it would be a "cobbly sandy loam", and if the cobble occupied 90% of the total solid volume it would be "cobble with trace sandy loam".
