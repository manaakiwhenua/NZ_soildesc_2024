...
Horizon boundaries comprise state changes over short distances. These should be reported as a direct measurement in the same way as horizon upper and lower boundary depths, by reporting the range either side of the boundary. Report in 0.1 centimetre increments (e.g. [0.5 cm]{.ceg}).

For rapid assessment, horizon boundaries can be classified using the schema in @tbl-hr-bdst.
...
Note that parent material origin (PMO), when summarised on a whole-profile basis, is the final defining factor of the soil Family [@webb2011]. 
