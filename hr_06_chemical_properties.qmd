...
Field tests do have limited precision and accuracy as compared to their laboratory equivalents, due to their simplicity and short observation times. Where field tests are not conclusive, laboratory testing may still be warranted.

Field tests should be performed down-profile, with at least one sample tested per horizon. Samples should be taken from the mid-point of each horizon, extracted with clean hands or tools to avoid cross-contamination. Additional samples in transition zones may also be of interest. Properties within the upper part of the first B horizon can be diagnostic in soil classification.

::: {.callout-warning appearance="minimal" collapse="false" title="Chemical safety"}
Many of the tests in this chapter require the use of reagents that have safety risks. Users should be familiar with the safety requirements for any chemicals used in the field and appropriately trained in their safe storage, transport and use [@worksafenewzealand2018].
:::
...
For classifying using the NZSC, particular tests down to or at a certain depth may be helpful, and may be necessary where laboratory analyses will not be completed. At the soil order level, these are:

-   **Gley Soils:** Detectable ferrous ions (Fe^2+^) in the soil solution within the top 30 cm, signifying a reduced/anoxic environment (@sec-ft-rdx). A slaking test may help confirm absence of an underlying duripan (@sec-ft-slak)
...
-   **Oxidic Soils:** No pH response to sodium fluoride within the diagnostic oxidic horizon(s) (@sec-ft-naf). Oxidic horizons may also slake rapidly under field conditions (@sec-ft-slak).
-   **Pallic Soils:** Where a calcareous horizon is part of the profile, confirmation of its presence with 10% HCl (@sec-ft-ca). No pH response to sodium fluoride in at the uppermost B horizon (@sec-ft-naf).
-   **Brown Soils:** at least moderate pH response to sodium fluoride in the uppermost subhorizon of the B horizon. (@sec-ft-naf)

pH, carbonate, reactive aluminium and salinity tests are also common diagnostic features at the group and subgroup level in the NZSC.
:::
...
1.  With clean hands or a small scoop, remove \~1–2 cm^3^ soil into a small container. Mix or crush the sample lightly to break open small peds.
...
-   Samples taken for laboratory analysis may change pH during transport, while in storage or during sample processing.
:::

### Field pH in water (pH~fw~) {#sec-ft-phfw}

pH~fw~ is measured with a field-ready electronic pH meter. Electronic meters are precise and can usually measure the full 0–14 pH range, but are sensitive to temperature fluctuations and must be regularly calibrated. Always refer to the device manual for specific maintenance requirements.
...
1.  With clean hands or a small scoop, remove \~1–2 cm^3^ soil into a small container. Mix with DI or DM water to a loose paste, ensuring that the mix is deep enough for the pH probe to be adequately covered.
2.  Insert the pH probe into the sample and wait for the reading to stabilise to 1 decimal place. Stability is indicated by no change for 10 seconds. Do not stir the sample with the probe.
...
Field pH in concentrated hydrogen peroxide (30% H~2~O~2~) is paired with field pH in water to diagnose the presence of FeS~2~ (iron sulfide, or pyrite) and thus the presence of sulfidic soil materials. A significant drop in pH~fox~ vs pH~fw~ is considered diagnostic; see the discussion in @sullivan2018, section A1.4.4 for further details.
...
1.  With clean hands or a small scoop, remove \~1–2 cm^3^ soil into each tube.
2.  Add enough H~2~O~2~ to cover each sample, and stir briefly. **Do not cap and shake the tubes**.
3.  Allow up to 15 minutes for any reaction to occur, especially in cool weather. Placing the rack in the sun may help progress the reaction. Note that strong reactions can sometimes occur almost instantly. Observe the samples from a short distance away and do not peer into the tubes.
...
-   In the field, H~2~O~2~ containers should be kept out of direct sunlight and preferably in a small cooler, as excessive heat will cause the H~2~O~2~ to break down, releasing oxygen. This can cause its container to rupture or spray it contents when opened. Only take small containers into the field, and monitor them for signs of elevated internal pressure (e.g. bulging). Make sure all containers are clearly labelled.
:::
...
The soil solution's EC~f~ is an estimate of its soluble salt content, which can affect plant nutrition and water uptake. Salinity is a diagnostic criterion in the NZSC, used within the Gley, Semi-Arid, Recent and Raw Soil orders. In New Zealand, a 1:5 ratio of soil to water is used in the laboratory, and so this must be matched in the field.
...
2.  Cap and shake each sample vigorously for 30 seconds, then allow the samples to settle for a few minutes.
...
While large chunks of calcium carbonate (CaCO~3~) in the form of shells or calcareous rocks are easy to spot, CaCO~3~ in the fine earth is not, and there can also be some doubt about the composition of secondary features. This test should be considered for use when working in areas where calcareous rocks occur near the land surface, where field pH \> 7, or when white or pale concentrations or coatings are observed - such as in some Pallic Soils and Semiarid Soils.

Calcium carbonate reacts with acidic solutions in a neutralisation reaction, releasing water, carbon dioxide and heat as the carbonate becomes a salt (calcium chloride). The reaction is rapid and causes a soil sample to 'fizz' visibly. 10% (or 1 *M*) HCl is used for this test as it is stable, cheap and relatively safe while still reacting vigorously.

Note that dolomitic carbonates may not react noticeably to this strength of acid in the field, and may require laboratory identification.
...
1.  With clean hands or a small scoop, remove \~1–2 cm^3^ soil into a small container. Lightly crush to break up peds and/or concentrations.
...
HCl can also be applied directly to a soil profile exposure, but be careful not to include the tested patches in any soil samples.
...
-   Do not transport large quantities of HCl, even dilute. A 25–50 mL bottle will normally last for weeks of fieldwork.
-   Make sure HCl containers are clearly labelled and corrosion-resistant.
:::
...
While @milne1995 and some other publications [@rayment2011; @kalra1991] have attempted to use the degree of fizzing to estimate CaCO~3~ percentage content, the estimates and descriptions of fizzing vary and are not considered highly reliable. If a precise numeric estimate of CaCO~3~ percentage is required, it should be measured under laboratory conditions.
:::

### Manganese (Mn~f~) {#sec-ft-mn}

Manganese-rich concentrations in soil are dark to black (sometimes bluish-black , e.g. pyrolusite [@churchman2012]) in colour and often very small, so may be missed or mistaken for other minerals. Manganese oxides react to H~2~O~2~ in a similar fashion to iron sulfide (see @sec-ft-phfox). This test uses a safer dilute H~2~O~2~ solution. This test should be considered for use where dark flecks are observed in the soil profile, particularly under imperfect to poor drainage conditions (see @sec-in-hdrng). Larger manganese oxide concretions will also generate a brown streak on a streak plate or white paper (whereas charcoal, in contrast, will generate a black streak).
...
The solution and treated paper both degrade over time and should not be stored for long periods.

*Procedure*

1.  With clean hands or a small scoop, place \~0.5 cm^3^ soil on a piece of pre-treated filter paper. Lightly crush to break up peds and put the soil in firm contact with the paper.
...
-   NaF is toxic and an irritant to skin and eyes. Wear safety glasses and gloves when handling, and in a field situation make sure that there is access to plenty of clean water for rinsing.
...
dat_ft_naf <-
  tribble(~Code, ~Name, ~Description, ~Colour, 
          ...
          'H', 'Strong reaction', 'Paper turns dark pink, usually within 10 seconds', '#A11844'
  ) |>
...
dat_ft_naf <-
  tribble(~Code, ~Name, ~Description, ~Colour, 
          ...
          'H', 'Strong reaction', 'Paper turns dark pink, usually within 10 seconds', ''
  )
...
Reactivity ratings used to interpret the NaF~f~ test have been inconsistently presented over the decades since its original publication, with a tendency to get more complicated over time. The reactivity ratings presented in @tbl-ft-naf is a return towards the simpler schemas seen in early reporting.

While a strong correlation can be made between phosphate retention and NaF reaction intensity within younger tephra-derived soils (Allophanic and Pumice Soil orders), the correlation does not hold across all soils.
:::

### Reducing conditions {#sec-ft-rdx}

Reducing conditions can be suggested by low-chroma soil colours (see @nte-lowch), redox mottling and other indicators of poor drainage. More confident diagnoses can be made by including field testing for reduced iron (Fe^2+^) as an indicator of persistently reducing conditions. Rapid testing for Fe^2+^ can be accomplished using 2,2'-bipyridine (previously/also known as ⍺-⍺-dipyridyl) dye, which turns pinkish red in the presence of Fe^2+^ [@childs1981; @vepraskas2015]. Test strips are also commercially available and may prove easier to use in the field [@berkowitz2017]. Note that these tests should only be conducted on wet or moist soils; soils that are in a slightly moist to dry state will not be experiencing reducing conditions. This test may be useful for distinguishing a reduced horizon from a low-chroma eluvial one (see also @sec-hor-e).

*Procedure*

1.  With clean hands or a small plastic scoop, remove \~1–2 cm^3^ soil into a small container or a surface with a white background.
2.  Depending on method:
    a.  If using indicator strips, press the strip gently against a freshly exposed sample surface for a few seconds, then remove and check for colour change.
    b.  If using 2,2'-bipyridine dye, apply a few drops to a freshly exposed sample surface and watch for a colour change.
3.  Record the presence or absence of a reaction (e.g. [40 cm - ✔]{.ceg}).

In both cases work quickly because, if reduced, the sample will oxidise rapidly on excavation.

::: {#wrn-aafp .callout-warning appearance="minimal" collapse="false" title="False positives"}
Recent contact with steel tools, previous application of 10% HCl when testing for carbonates (@sec-ft-ca), and exposure to intense sunlight after the dye has been applied can all cause false positive readings for this test [@vepraskas2015].
:::

::: {#wrn-aasaf .callout-warning appearance="minimal" collapse="true" title="Safe handling of 2,2'-Bipyridine solution"}
Some manuals have suggested the use of a spray bottle to apply 2,2'-Bipyridine to an exposure or extracted sample. This is not recommended due to the chemical's toxicity. See @nationalcenterforbiotechnologyinformation2024 for further details. A squirt bottle that does not emit a mist may be a safer choice.
:::

::: {#nte-reduc .callout-note appearance="minimal" collapse="true" title="Other methods of assessing reduction"}
Current best practice for research-grade monitoring of redox potential in soils involves the use of Indicator of Reduction In Soils (IRIS) devices [@sapkota2022], sometimes in concert with electronic sensors. IRIS devices require specialist construction, installation for several weeks and careful post-processing.
:::

### Water repellence {#sec-ft-wtrep}

Water repellence in soils results from hydrophobic organic compounds (e.g. plant leaf waxes) accumulating under a specific range of soil moisture conditions [@dekker2009; @soilsurveystaffFieldBookDescribing2024]. Repellency is thus locally and seasonally variable, and usually confined to surface layers. Water repellence is most commonly assessed by observing absorbence of distilled water and/or ethanol.

*Equipment*

-   Deionised (DI) or demineralised (DM) water, stored in a small dropper bottle
-   2-molar ethanol, stored in a small dropper bottle
-   Small sample containers or plastic tray

*Procedure*

This test is only reliable on dry soils.

1.  With clean hands or a small scoop, remove \~2–3 cm^3^ soil into a small container. Leave the sample intact.
2.  Use a dropper or syringe to place 3–5 well-separated drops of distilled water onto the soil surface, from a distance of no more than 5 mm. Measure the median time it takes for the drops to be absorbed.
3.  If the water drops take longer than 10 seconds to disappear, repeat the test with a fresh sample and use ethanol.
4.  Record the repellence rating according to @tbl-ft-wrep.

Note that water repellence is affected by ambient temperature [@ncst2024].

```{r}
#| label: tbl-ft-wrep
#| tbl-cap: "water repellency ratings"

dat_ft_wrep <-
  tribble(~Code, ~Name, ~Description, 
          'N', 'Not repellent',  'Water absorbs within 10 seconds', 
          'R', 'Repellent', 'Water does not absorb within 10 seconds. Ethanol absorbs within 10 seconds', 
          'S', 'Strongly repellent', 'Ethanol not absorbed within 10 seconds'
  )
  
tbl_ft_wrep <- gt(dat_ft_wrep) |>
  tab_options(
    column_labels.font.weight = 'bold', 
    heading.title.font.weight = 'bold',
    table.align = 'center',
...
2.  Carefully add one or two dry peds from each pedal soil horizon (or 1–2 cm^3^ of minimally disturbed material from a massive horizon), working clockwise around the dish. Minimise disturbing the water.
...
dat_ft_slak <-
  tribble(~Code, ~Name, ~Description,
          ...
          'C', 'Complete slaking', 'Peds have collapsed into a pile of separated grains, and little to no ped structure remains visible') 
