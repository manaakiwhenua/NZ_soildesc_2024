...
[Field tests are useful for...]{.todo} - confident classification - instant results - sample quality control - potentially cost-minimising
...
pH~fw~ is measured with a field-ready electronic pH meter. Electronic meters are precise, but sensitive to temperature fluctuations and must be regularly calibrated. Always refer to the device manual for specific calibration instructions and information about device tolerances and optimum conditions for performance. As well as the meter, this test requires a set of small jars or tubes for holding samples, deionised (DI) or demineralised (DM) water, and mixing sticks.
...
2.  Insert the probe (and ideally a connected temperature sensor) into the sample and wait for the reading to stabilise to 1 decimal place (No change for 10s). Do not stir the sample with the probe.
...
This test requires a calibrated electronic pH meter (preferably with a temperature sensor), a rack of heat-resistant 50 mL centrifuge tubes, and some stirring sticks.
...
4.  Where reactions occur, note their violence using the scale in @tbl-ft-foxrx.
...
This test detects the presence of reactive hydroxy-aluminium sites on soil minerals. In New Zealand, these sites are most often associated with the presence of the minerals allophane and imogolite. These minerals are abundant in tephra-derived soils, and can also accumulate in the B horizons of Podzols. Reactive hydroxy-aluminium sites can also be found on aluminium-humus complexes, especially in podzolic B horizons.

The test uses filter paper, pre-soaked in phenolpthalein indicator and dried. The indicator turns pink at \~pH \> 9. Adding 1 *M* sodium fluoride (NaF) solution to a soil sample with considerable reactive hydroxy-aluminium sites will cause the solution pH to rise and stain the paper.

::: {.callout-important collapse="false" title="Use caution interpreting the NaF~f~ test"}
-   The presence of Calcium carbonate will cause a strong false positive for this test. When in doubt, pre-screening with HCl or a pH test is recommended to avoid confusion.
...
2.  Using a dropper bottle, add a drop or two of 1 *M* NaF (enough to wet the sample but not flood it) and wait 2 minutes.
...
Scales used to interpret the NaF~f~ test have been inconsistently presented over the decades since its original publication, with a tendency to get more complicated over time. The scale presented above is a return towards the simpler schemas seen in early reporting. No reliable connection between allophane content and reaction intensity can be made using available data.
:::

### Reducing conditions {#sec-ft-rdx}

Reducing conditions can be suggested by low-chroma soil colours, redox mottling and other indicators of poor drainage (XREF). More confident diagnoses can be made by including field testing for reduced iron (Fe^2+^) as an indicator of persistently reducing conditions. Rapid testing for Fe^2+^ can be accomplished using ⍺-⍺-dipyridyl dye, which turns pinkish red in the presence of Fe^2+^. Test strips are also commercially available and may prove easier to use in the field [@berkowitz2017]. Note that these tests should only be conducted on wet soils; soils that are in a moist to dry condition will not be experiencing reducing conditions.
...
Current best practice for research-grade monitoring of redox potential in soils involves the use of IRIS indicator devices [@sapkota2022]. These require specialist construction, installation for several weeks and careful post-processing.

::: {.callout-note collapse="true" title="Safe use of ⍺-⍺-dipridyl solution"}
