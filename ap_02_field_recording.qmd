...
Recording field data in a regular format helps ensure that consistently high standards are kept. Well-structured and complete soil profile descriptions are key supports for subsequent analytical work, including mapping tasks and analysis of laboratory data.
...
![A minimal demonstration field sheet, suitable for rapid recording of low-detail descriptions.](data/simple_sheet_filled.png){width=70% .lightbox alt="A demonstration minimalist field sheet, filled out with some data from an NSD site completed in the 1970s."}
...
Using a purpose-built app for data entry can reduce the double handling inherent to recording hardcopy data. Apps do come with design, deployment and maintenance overheads that need to be taken into account, as well as a significant learning curve for paper-adapted users. 

At the time of writing there is no consumer-grade app available for recording NZ soils field data, but the ESRI and QGIS spatial software ecosystems have field data entry frameworks that can be adapted for soils fieldwork.
