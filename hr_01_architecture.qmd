...
Soil horizon architecture comprises presence and arrangement of fine earth, stones, roots, and other materials. The presence and connectivity of voids is also described.

## Fine-earth structure {#sec-hr-stru}

Structure refers to the physical arrangement of the fine-earth fraction (\< 2 mm). Structure is determined by mineralogy, chemistry, the action of soil biota and land use history.
...
Rock fragments in soil horizons (\> 2 mm) provide information about soil development, particularly mode of emplacement in depositional soils. They also restrict the soil's water holding capacity, and when large and common enough can restrict land use and management practices.

For rapid assessment, estimate total rock fragment abundance as an integer percentage, recording [0%]{.ceg} when confirming absence.

For routine assessment, estimate abundance of small, medium and large stones separately (@tbl-hr-rockfrsz). Optionally, append dominant lithology class of each size fraction using @tbl-ss-pm and shape using @tbl-hr-rockfrshp.
...
#| tbl-cap: "Coarse fragment size classes for routine assessment"
#| classes : no-stripe
...
For detailed assessment, provide separate estimates for roots \< 2 mm, 2 - 10 mm and \> 10 mm. Record as whole percentages, confirming [0%]{.ceg} where no roots are observable in a given size range. The data may be classified using @tbl-hr-rtab.
...
Horizon voids exclude surface cracks, which are noted in @sec-pr-crack and fine pores, which are described below. Filled voids are described in [secondary biological features]{.todo}

For rapid assessment, simply report the percentage of the horizon volume occupied by void space.
